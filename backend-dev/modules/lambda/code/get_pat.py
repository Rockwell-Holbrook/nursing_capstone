...
def lambda_handler(event,context):
    ...
    patient = {
        "id": pat['Items'][0]['id'],
        "recordings": [],
        "date_modified": pat['Items'][0]['date_modified'],
        "created_by": pat['Items'][0]['created_by'],
        "tags": pat['Items'][0]['tags'],
        "abnormal": pat['Items'][0]['abnormal']
    }
    
    for recording in pat['Items'][0]['recordings']:
        # Query recording DB
        rec = REC_DB.query(
            KeyConditionExpression=Key('id').eq(recording)
        )
        
        print(rec)

        # Create presigned URL
        pre_url = get_rec_url(recording)

        patient['recordings'].append(
            {
                "id": recording,
                "url": pre_url,
